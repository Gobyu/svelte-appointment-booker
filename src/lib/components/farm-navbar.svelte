<script lang="ts">
	...
	const isFarmHome = $derived(currentPath === '/farm/home');

	const rightButtonHref = $derived(isFarmHome ? '/' : '/farm/home');
	const rightButtonLabel = $derived(isFarmHome ? '← Happy Family' : '← Happy Family Farm');
	...
	<div class="mx-auto max-w-6xl px-4">
		...
		hover:text-indigo-600 dark:text-gray-100 dark:hover:text-indigo-400"
			...
			<div class="hidden items-center gap-6 md:flex">
				<a
					href="/farm/about"
					class="rounded-xl px-4 py-2 text-sm font-medium text-gray-800 transition
			hover:scale-110 hover:text-indigo-700
			dark:text-gray-100 dark:hover:text-indigo-400"
					class:font-semibold={isActive('/farm/about')}
					aria-current={isActive('/farm/about') ? 'page' : undefined}
				>
					About
				</a>

				<a
					href="/farm/products_services"
					class="rounded-xl px-4 py-2 text-sm font-medium text-gray-800 transition
			hover:scale-110 hover:text-indigo-700
			dark:text-gray-100 dark:hover:text-indigo-400"
					class:font-semibold={isActive('/farm/products_services')}
					aria-current={isActive('/farm/products_services') ? 'page' : undefined}
				>
					Products/Services
				</a>
			...
			hover:scale-110 hover:text-indigo-700
			dark:text-gray-100 dark:hover:text-indigo-400"
					class:font-semibold={isActive('/farm/contact')}
					aria-current={isActive('/farm/contact') ? 'page' : undefined}
				>
					Contact Us
				</a>
			</div>
